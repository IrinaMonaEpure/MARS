...
def gen(cfg:Config):
    ...
    node_embedding = distribute_nodes_uniformly(cfg.network.n_nodes, label_prefix='u') # I add a prefix to node names so you don't get a node and affiliation called the same thing.

    layers = []

    for l in range(cfg.network.n_layers):
        affiliation_embedding = distribute_nodes_uniformly(cfg.network.n_affiliations[l], label_prefix='A')
        ...
        for node in node_embedding:
            ai = choose_affiliation(node_embedding[node], affiliation_embedding, cfg.connection.xi, cfg.connection.r_0, cfg.connection.s)
            H.add_edge(ai, node)
